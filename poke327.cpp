...
void new_hiker()
{
  ...
  c->name = trainer_names[rand() % 30];
  
  int thingwhat;
  do {
    random_pokemon p = random_pokemon();
    c->lineup.push_back(p);
    thingwhat = rand();
  } while (thingwhat % 5 < 3 && c->lineup.size() < 6);// TODO REPLACE 0 WITH 3
...
void new_rival()
{
  ...
  do {
    random_pokemon p = random_pokemon();
    c->lineup.push_back(p);
  } while (rand() % 5 < 3 && c->lineup.size() < 6); // TODO REPLACE 0 WITH 3
  heap_insert(&world.cur_map->turn, c);
}
...
void new_swimmer()
{
  ...
  do {
    random_pokemon p = random_pokemon();
    c->lineup.push_back(p);
  } while (rand() % 5 < 3 && c->lineup.size() < 6);
  heap_insert(&world.cur_map->turn, c);
}
